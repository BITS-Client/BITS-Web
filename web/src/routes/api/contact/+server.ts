...
export const POST: RequestHandler = async ({ request, getClientAddress }) => {
	try {
		...
		const CONTACT_TO_EMAIL = env.PUBLIC_CONTACT_TO_EMAIL;
		const CONTACT_FROM_EMAIL = env.PUBLIC_CONTACT_FROM_EMAIL;
		...
		if (!RESEND_API_KEY || !CONTACT_TO_EMAIL) {
			console.warn('RESEND_API_KEY or env.PUBLIC_CONTACT_TO_EMAIL not set, using fallback values');
		}
