...
- **Dependabot**: `.github/dependabot.yml` — weekly `github-actions` (Mon 02:30) + `npm` for `/` (Mon 03:00) — covers all pnpm workspace packages (`web`, `studio`, root) via the root lockfile — grouped dev-deps minor/patch, ignores `typescript` major.
